<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_feed|template/default/header|template/default/space_status|template/default/space_menu|template/default/space_feed_li|template/default/space_feed_li|template/default/space_feed_li|template/default/footer', '1365987876', 'template/default/space_feed');?><?php if(empty($_TPL['getmore'])) { ?>	
<?php $_TPL['titles'] = array('首頁'); ?>
...
<ul class="u_setting">
<li class="dropmenu" id="usettingli" onclick="showMenu(this.id)"><a href="javascript:;">個人設置 <img src="image/more.gif" align="absmiddle"></a></li>
</ul>
<ul id="usettingli_menu" class="dropmenu_drop" style="display:none;">
<li><a href="cp.php?ac=avatar">我的頭像</a></li>
<li><a href="cp.php?ac=profile">個人資料</a></li>
<li><a href="cp.php?ac=theme">主頁風格</a></li>
<li><a href="cp.php?ac=credit">我的積分</a></li>
<?php if($_SCONFIG['sendmailday']) { ?>
<li><a href="cp.php?ac=sendmail">郵件提醒</a></li>
<?php } ?>
<li><a href="cp.php?ac=privacy">隱私篩選</a></li>
<?php if(checkperm('admin')) { ?>
<li><a href="admincp.php">站點管理</a></li>
<?php } ?>
<?php if(checkperm('allowstat')) { ?>
<li><a href="do.php?ac=stat">趨勢統計</a></li>
<?php } ?>
</ul>
</td>
<td valign="top">
<h3 class="index_name">
<div class="r_option">
<?php if($_SGLOBAL['session']['magichidden']) { ?>當前隱身<?php } else { ?>當前在線<?php } ?>

<?php if($_SGLOBAL['magic']['invisible']) { ?>
<?php if($_SGLOBAL['session']['magichidden']) { ?>
<img src="image/magic/invisible.small.gif" class="magicicon"><a id="a_magic_appear" href="cp.php?ac=magic&op=appear" onclick="ajaxmenu(event,this.id)" class="gray">我要在線</a>
<?php } else { ?>
<img src="image/magic/invisible.small.gif" alt="<?=$_SGLOBAL['magic']['invisible']?>" class="magicicon"><a id="a_magic_invisible" href="magic.php?mid=invisible" onclick="ajaxmenu(event,this.id,1)" class="gray">我要隱身</a>
<?php } ?>
<?php } ?>
</div>
...
已有 <?=$space['viewnum']?> 人次訪問, <?=$space['credit']?> 個積分, <?=$space['experience']?> 個經驗
</div>
...
<div id="mood_statusinput" class="statusinput"><textarea name="message" id="mood_message" onclick="statusFace();" onkeydown="if(event.keyCode == 13 ){ event.returnValue=false;event.cancel = true;$('mood_add').click();$('mood_message').value='';this.blur(); };" >你可以更新狀態, 讓好友們知道你在做什麼...</textarea></div>
...
function statusFace() {
if($('mood_message').value == '你可以更新狀態, 讓好友們知道你在做什麼...'){
$('mood_message').value = '';
}
...
$('mood_message').value = '你可以更新狀態, 讓好友們知道你在做什麼...';
}
...
<?php if($space['allnum']) { ?>
<div class="mgs_list">
<?php if($space['notenum']) { ?><div><img src="image/icon/notice.gif"><a href="space.php?do=notice"><strong><?=$space['notenum']?></strong> 條新通知</a></div><?php } ?>
<?php if($space['addfriendnum']) { ?><div><img src="image/icon/friend.gif" alt="" /><a href="cp.php?ac=friend&op=request"><strong><?=$space['addfriendnum']?></strong> 個好友請求</a></div><?php } ?>
<?php if($space['mtaginvitenum']) { ?><div><img src="image/icon/mtag.gif" alt="" /><a href="cp.php?ac=mtag&op=mtaginvite"><strong><?=$space['mtaginvitenum']?></strong> 個群組邀請</a></div><?php } ?>
<?php if($space['eventinvitenum']) { ?><div><img src="image/icon/event.gif" alt="" /><a href="cp.php?ac=event&op=eventinvite"><strong><?=$space['eventinvitenum']?></strong> 個活動邀請</a></div><?php } ?>
<?php if($space['myinvitenum']) { ?><div><img src="image/icon/userapp.gif" alt="" /><a href="space.php?do=notice&view=userapp"><strong><?=$space['myinvitenum']?></strong> 個應用消息</a></div><?php } ?>
<?php if($space['pmnum']) { ?><div><img src="image/icon/pm.gif" alt="" /><a href="space.php?do=pm"><strong><?=$space['pmnum']?></strong> 條新短消息</a></div><?php } ?>
<?php if($space['pokenum']) { ?><div><img src="image/icon/poke.gif" alt="" /><a href="cp.php?ac=poke"><strong> <?=$space['pokenum']?></strong> 個新招呼</a></div><?php } ?>
<?php if($space['reportnum']) { ?><div><img src="image/icon/report.gif" alt="" /><a href="admincp.php?ac=report"><strong><?=$space['reportnum']?></strong> 個舉報</a></div><?php } ?>
<?php if($space['namestatusnum']) { ?><div><img src="image/icon/profile.gif" alt="" /><a href="admincp.php?ac=space&perpage=20&namestatus=0&searchsubmit=1"><strong><?=$space['namestatusnum']?></strong> 個待認證用戶</a></div><?php } ?>
<?php if($space['eventverifynum']) { ?><div><img src="image/icon/event.gif" alt="" /><a href="admincp.php?ac=event&perpage=20&grade=0&searchsubmit=1"><strong><?=$space['eventverifynum']?></strong> 個待審核活動</a></div><?php } ?>
</div>
<?php } ?>
...
<div class="notice">您好，<?=$_SN[$space['uid']]?>，歡迎加入我們。有新任務等著您，挺簡單的，趕快來參加吧</div>
<img src="<?=$task['image']?>" alt="" class="icon" />
<h3><a href="cp.php?ac=task&op=do&taskid=<?=$task['taskid']?>"><?=$task['name']?></a></h3>
<p>可獲得 <span class="num"><?=$task['credit']?></span> 積分</p>
</div>
</div>
</div></div></div></div><br>
<?php } ?>
...
<ul class="tabs">
<?php if($space['friendnum']) { ?><li<?=$actives['we']?>><a href="space.php?do=home&view=we"><span>好友動態</span></a></li><?php } ?>
<li<?=$actives['hot']?>><a href="space.php?do=home&view=hot"><span>熱門推薦</span></a></li>
<li<?=$actives['me']?>><a href="space.php?do=home&view=me"><span>我的動態</span></a></li>
<li<?=$actives['all']?>><a href="space.php?do=home&view=all"><span>全站動態</span></a></li>
</ul>
</div>
<?php } elseif($space['uid']) { ?>

<?php $_TPL['spacetitle'] = "動態";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=feed&view=me\">TA的近期動態</a>"; ?>
...
<?php if($hotlist) { ?>
<a href="space.php?do=home&view=hot" class="r_option">&raquo; 查看更多熱點</a>
<h4 class="feedtime" style="margin-top:5px;">近期熱點推薦</h4>
<ul>
...
<a href="cp.php?ac=feed&op=menu&feedid=<?=$value['feedid']?>" class="float_more" id="a_feed_menu_<?=$value['feedid']?>"  onmouseover="feed_menu(<?=$value['feedid']?>,1);" onmouseout="feed_menu(<?=$value['feedid']?>,0);" onclick="ajaxmenu(event, this.id)" title="顯示更多選項" style="display:none;">菜單</a>
<?php } ?>
<a class="type" href="space.php?uid=<?=$_GET['uid']?>&do=feed&view=<?=$_GET['view']?>&appid=<?=$value['appid']?>&icon=<?=$value['icon']?>" title="只看此類動態"><img src="<?=$value['icon_image']?>" /></a>
...
(<a href="javascript:;" onclick="docomment_get('docomment_<?=$value['id']?>', 1);" id="do_a_op_<?=$value['id']?>">回復</a>)
<?php } elseif(in_array($value['idtype'], array('blogid','picid','sid','pid','eventid'))) { ?>
(<a href="javascript:;" onclick="feedcomment_get(<?=$value['feedid']?>);" id="feedcomment_a_op_<?=$value['feedid']?>">評論</a>)
<?php } ?>
<?php } ?>
...
<div class="media">
<img src="image/vd.gif" alt="點擊播放" onclick="javascript:showFlash('<?=$value['body_data']['host']?>', '<?=$value['body_data']['flashvar']?>', this, '<?=$value['feedid']?>');" style="cursor:pointer;" />
</div>
<?php } elseif($value['thisapp'] && !empty($value['body_data']['musicvar'])) { ?>
<div class="media">
<img src="image/music.gif" alt="點擊播放" onclick="javascript:showFlash('music', '<?=$value['body_data']['musicvar']?>', this, '<?=$value['feedid']?>');" style="cursor:pointer;" />
</div>
<?php } elseif($value['thisapp'] && !empty($value['body_data']['flashaddr'])) { ?>
<div class="media">
<img src="image/flash.gif" alt="點擊查看" onclick="javascript:showFlash('flash', '<?=$value['body_data']['flashaddr']?>', this, '<?=$value['feedid']?>');" style="cursor:pointer;" />
</div>
<?php } ?>
...
<div id="appfeed_open_<?=$value['feedid']?>"><a href="javascript:;" id="feed_a_more_<?=$value['feedid']?>" onclick="feed_more_show('<?=$value['feedid']?>');">&raquo; 更多動態(<?=$hiddenfeed_num[$value['icon']]?>)</a></div>
...
根據您的<a href="cp.php?ac=privacy&op=view">篩選設置</a>，有 <?=$filtercount?> 條動態被屏蔽 (<a href="javascript:;" onclick="filter_more(<?=$start?>);" id="a_feed_privacy_more">點擊查看</a>)
</div>
<div id="feed_filter_div_<?=$start?>" class="enter-content" style="display:none;">
<h4 class="feedtime">以下是被屏蔽的動態</h4>
<ul>
...
<a href="javascript:;" onclick="feed_more();" id="a_feed_more">查看更多動態</a>
</div>
<?php } ?>
...
<div class="gray">已有 <span class="num"><?=$value['joinnum']?></span> 人參與</div>
</div>
<?php } } ?>
</div>
</div></div></div></div>
...
<p<?php if($ols[$value['uid']]) { ?> class="online_icon_p" title="在線"<?php } ?>><a href="space.php?uid=<?=$value['uid']?>" title="<?=$_SN[$value['uid']]?>"><?=$_SN[$value['uid']]?></a></p>
...
<p<?php if($ols[$value['vuid']]) { ?> class="online_icon_p" title="在線"<?php } ?>><a href="space.php?uid=<?=$value['vuid']?>" title="<?=$_SN[$value['vuid']]?>"><?=$_SN[$value['vuid']]?></a></p>
<p class="gray"><?php echo sgmdate('n月j日',$value[dateline],1); ?></p>
<?php } ?>
</li>
<?php } } ?>
...
<div class="avatar48"><a href="space.php?uid=<?=$value['uid']?>"><?php echo avatar($value[uid],small); ?></a></div>
<p<?php if($ols[$value['uid']]) { ?> class="online_icon_p" title="在線"<?php } ?>><a href="space.php?uid=<?=$value['uid']?>" title="<?=$_SN[$value['uid']]?>"><?=$_SN[$value['uid']]?></a></p>
<p class="gray"><?php if($value['lastactivity']) { ?><?php echo sgmdate('H:i',$value[lastactivity],1); ?><?php } else { ?>熱度(<?=$value['num']?>)<?php } ?></p>
</li>
<?php } } ?>
</ul>
</div>
...
<p>
<a href="cp.php?ac=friend&op=search">高級搜索</a><span class="pipe">|</span>
<a href="cp.php?ac=friend&op=find">可能認識的人</a><span class="pipe">|</span>
<a href="cp.php?ac=invite">邀請好友</a></p>
</div></div></div></div>
</div>
...
html = '<h4 class="feedtime">以下是新讀取的動態</h4>' + s;
