...
/*
	...
	$Id: function_pronotice.php 13245 2009-08-25 02:01:40Z liguode $
*/
...
//添加博客
function pronotice_post($POST, $olds=array()) {
	...
	if(!empty($POST['picids'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if(empty($titlepic) && $value['thumb']) {
				...
				$pronoticearr['picflag'] = $value['remote']?2:1;
			}
	...
	if(empty($titlepic)) {
		...
		$pronoticearr['picflag'] = 0;
	}
	$pronoticearr['pic'] = $titlepic;
	
	//热度
	if(checkperm('managepronotice')) {
		$pronoticearr['hot'] = intval($POST['hot']);
	}
	
	if($olds['pnid']) {
		//更新
		$pnid = $olds['pnid'];
		updatetable('pronotice', $pronoticearr, array('pnid'=>$pnid));
		
		$fuids = array();
		
		$pronoticearr['uid'] = $olds['uid'];
		$pronoticearr['username'] = $olds['username'];
	} else {
		//参与热闹
		$pronoticearr['topicid'] = topic_check($POST['topicid'], 'pronotice');

		$pronoticearr['uid'] = $_SGLOBAL['supe_uid'];
		$pronoticearr['username'] = $_SGLOBAL['supe_username'];
		$pronoticearr['dateline'] = empty($POST['dateline'])?$_SGLOBAL['timestamp']:$POST['dateline'];
		$pnid = inserttable('pronotice', $pronoticearr, 1);
	}
	
	$pronoticearr['pnid'] = $pnid;
	...
	if($POST['tag'] != $oldtagstr) {
		if(!empty($olds['tag'])) {
			//先把以前的给清理掉
			$oldtags = array();
			$query = $_SGLOBAL['db']->query("SELECT tagid, pnid FROM ".tname('tagpronotice')." WHERE pnid='$pnid'");
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$oldtags[] = $value['tagid'];
			}
			if($oldtags) {
				$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET pronoticenum=pronoticenum-1 WHERE tagid IN (".simplode($oldtags).")");
				$_SGLOBAL['db']->query("DELETE FROM ".tname('tagpronotice')." WHERE pnid='$pnid'");
			}
		}
		$tagarr = tag_batch($pnid, $POST['tag']);
		//更新附表中的tag
	...
	if($olds) {
		//更新
		updatetable('pronoticefield', $fieldarr, array('pnid'=>$pnid));
	} else {
		$fieldarr['pnid'] = $pnid;
		$fieldarr['uid'] = $pronoticearr['uid'];
		inserttable('pronoticefield', $fieldarr);
	}
	...
	if($isself) {
		if($olds) {
			...
			if(empty($space['pronoticenum'])) {
				$space['pronoticenum'] = getcount('pronotice', array('uid'=>$space['uid']));
				$pronoticenumsql = "pronoticenum=".$space['pronoticenum'];
			} else {
				$pronoticenumsql = 'pronoticenum=pronoticenum+1';
			}
			//积分
			$reward = getreward('publishpronotice', 0);
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$pronoticenumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
			
			//统计
			updatestat('pronotice');
		}
	}
	...
	if($POST['makefeed']) {
		...
		feed_publish($pnid, 'pnid', $olds?0:1);
	}
	
	//热闹
	if(empty($olds) && $pronoticearr['topicid']) {
		topic_join($pronoticearr['topicid'], $_SGLOBAL['supe_uid'], $_SGLOBAL['supe_username']);
	}
...
function tag_batch($pnid, $tags) {
	...
	if($updatetagids) $_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET pronoticenum=pronoticenum+1 WHERE tagid IN (".simplode($updatetagids).")");
	$tagids = array_keys($tagarr);
	$inserts = array();
	foreach ($tagids as $tagid) {
		$inserts[] = "('$tagid','$pnid')";
	}
	if($inserts) $_SGLOBAL['db']->query("REPLACE INTO ".tname('tagpronotice')." (tagid,pnid) VALUES ".implode(',', $inserts));
...
//视频标签处理
function pronotice_bbcode($message) {
	$message = preg_replace("/\[flash\=?(media|real)*\](.+?)\[\/flash\]/ie", "pronotice_flash('\\2', '\\1')", $message);
	return $message;
}
//视频
function pronotice_flash($swf_url, $type='') {
	$width = '520';
	$height = '390';
