...
$reward = getreward('pronoticeimport', 0);
$siteurl = getsiteurl();
...
if(submitcheck('importsubmit')) {
	...
	$xmldata .= '<methodCall><methodName>metaWepronotice.getRecentPosts</methodName>';
	$xmldata .= '<params>';
	$xmldata .= '<param><value><string>pronotice</string></value></param>';
	...
	if(empty($result)) {
		showmessage('pronotice_import_no_result', '', 1, array(shtmlspecialchars($org_result)));
	}
	...
	if(!isset($ones['postid'])) {
		...
		showmessage('pronotice_import_no_data', '', 1, array($return));
	}
