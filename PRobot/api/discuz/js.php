...
END;
	...
	if(in_array($_GET['pagetype'], array('index', 'forumdisplay', 'viewthread')) && $side && $infosidestatus == 2) {
		$spacestr = '<div class="sidebox"><h4>競價排行</h4><ul class="avt_list avt_uname">';
	...
	if(($_GET['pagetype'] == 'profile' && $viewpro) || ($_GET['pagetype'] == 'viewthread')) {
		...
		if($updateuid) {
			...
			if($_GET['pagetype'] == 'profile' && $viewpro) {
				...
				if($albumnum)	$contstr = '<li><img src="'.$siteurl.'image/icon/album.gif"><a href="'.$siteurl.'space.php?uid='.$updateuid.'&do=album&view=me" target="_blank">'.$albumnum.'個相冊</a></li>';
				if($doingnum)	$contstr .= '<li><img src="'.$siteurl.'image/icon/doing.gif"> <a href="'.$siteurl.'space.php?uid='.$updateuid.'&do=doing&view=me" target="_blank">'.$doingnum.'條記錄</a></li>';
				if($blognum)	$contstr .= '<li><img src="'.$siteurl.'image/icon/blog.gif"> <a href="'.$siteurl.'space.php?uid='.$updateuid.'&do=blog&view=me" target="_blank">'.$blognum.'篇日誌</a></li>';
				if($threadnum)	$contstr .= '<li><img src="'.$siteurl.'image/icon/thread.gif"> <a href="'.$siteurl.'space.php?uid='.$updateuid.'&do=thread&view=me" target="_blank">'.$threadnum.'個話題</a></li>';
				if($tagspacenum)	$contstr .= '<li><img src="'.$siteurl.'image/icon/mtag.gif"> <a href="'.$siteurl.'space.php?do=mtag&uid='.$updateuid.'" target="_blank">'.$tagspacenum.'個群組</a></li>';
...
END;
				...
				if($pid && $avatarfeedstr && $updatetime < 86400) {
					...
					if(!isset($_SCOOKIE['viewuserid']) || !in_array($updateuid, explode(',', $_SCOOKIE['viewuserid']))) {
						...
						if(!empty($_GET['plugin'])) {
							print <<<END
$('authorfeed').innerHTML = '<span id="authornewfeed" onmouseover="showMenu(this.id);">有新動態</span>';
